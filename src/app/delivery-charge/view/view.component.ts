...
export class ViewComponent  implements OnInit
{
  ...
  constructor(
    ...
    ){SY.PAGE('DELIVERYCHARGE DETAILS');}
  ...
  REMOVE(IT:any)
  {
    this.UI.CONFIRM('CONFIRM DELETE',` ${IT.deliveryChargeId} will be deleted. Are you sure ?`).subscribe(r=>{
      console.log('r',r)
      if(r>=100){
        this.DELETE(IT);
      } 
  ...
  DELETE (IT:any) {
    const url = `${ZXAPI.DELIVARY.VIEW}/${IT.deliveryChargeId}`;
    this.loading = true; 
    ...
    try {
      this.MW.DELETE(url).subscribe(
        ...
        (error: any) => {
          this.UI.SAY('Error deleting delivery-charge');
        }
      ).add(() => {
        this.loading = false; 
      });
  ...
  EDIT(IT:any)
  {
    this.SY.OPEN('delivery-charge/edit',IT.deliveryChargeId);
  }
}
