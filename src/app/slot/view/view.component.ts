...
export class ViewComponent implements OnInit
{
  ...
  REMOVE(IT:any)
  {
    this.UI.CONFIRM('CONFIRM DELETE',` ${IT.slotId} will be deleted. Are you sure ?`).subscribe(r=>{
      console.log('r',r)
      if(r>=100){
        this.DELETE(IT);
      } 
  ...
  DELETE (IT:any) {
    const url = `${ZXAPI.SLOT.VIEW}/${IT.slotId}`;
    this.loading = true; 
    ...
    try {
      this.MW.DELETE(url).subscribe(
        ...
        (error: any) => {
          this.UI.SAY('Error deleting Slot');
        }
      ).add(() => {
        this.loading = false; 
      });
  ...
  EDIT(IT:any)
  {
    this.SY.OPEN('slot/edit',IT.slotId);
  }
}
